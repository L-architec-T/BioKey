...
if [[ "$ARCH" == "x64" ]]; then
  ...
  docker cp pcbu_build_container:/project/pkg/build/BioKey.AppImage ./pkg/build/BioKey-x64.AppImage
  ...
  docker run --name pcbu_build_container --cap-add=SYS_ADMIN --security-opt apparmor:unconfined --device /dev/fuse pcbu_builder
  docker cp pcbu_build_container:/project/pkg/build/BioKey.AppImage ./pkg/build/BioKey-arm64.AppImage
  docker container rm pcbu_build_container
else
...
chmod 777 ./pkg/build/BioKey*.AppImage
chmod +x ./pkg/build/BioKey*.AppImage
